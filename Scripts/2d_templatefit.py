...
################ CC estimator cut #####################
#template_correct = template_correct[np.where(template_correct[:,0]>0.3)[0],:]
#template_confused = template_confused[np.where(template_confused[:,0]>0.3)[0],:]
#template_electron = template_electron[np.where(template_electron[:,0]>0.3)[0],:]
#data_negative = data_negative[np.where(data_negative[:,0]>0.3)[0],:]
#data_positive = data_positive[np.where(data_positive[:,0]>0.3)[0],:]

######## create a graph ###############################
myfile = TFile ('histo.root','new')
TH_template_correct = TH2D("template_correct","template_correct", 100,0,1,100,0.1,1.5);
TH_template_confused = TH2D("template_confused","template_confused", 100,0,1,100,0.1,1.5);
TH_template_electron = TH2D("template_electron","template_electron", 100,0,1,100,0.1,1.5); 
TH_data_negative = TH2D("data_negative","data_negative", 100,0,1,100,0.1,1.5);
TH_data_positive = TH2D("data_positive","data_positive", 100,0,1,100,0.1,1.5);
