...
for protoniterate in range(binnumber):
        protonaccumulatecount=protonaccumulatecount+np.histogram(y_pred[0:testset1.shape[0],1],binnumber,range=(0,1),density=True)[0][binnumber-protoniterate-1]
...
for CCprotoniterate in range(binnumber):
        CCprotonaccumulatecount=CCprotonaccumulatecount+np.histogram(y_pred[testset1.shape[0]:y_pred.shape[0],1],binnumber,range=(0,1),density=True)[0][binnumber-CCprotoniterate-1]
...
for protoniterateMvA in range(binnumber):
        protonaccumulatecountMvA=protonaccumulatecountMvA+np.histogram(testsetMvA[0:testset1.shape[0]],binnumber,range=(0,1),density=True)[0][binnumber-protoniterateMvA-1]
...
for CCprotoniterateMvA in range(binnumber):
        CCprotonaccumulatecountMvA=CCprotonaccumulatecountMvA+np.histogram(testsetMvA[testset1.shape[0]:y_pred.shape[0]],binnumber,range=(0,1),density=True)[0][binnumber-CCprotoniterateMvA-1]
...
plt.hist(y_pred[0:testset1.shape[0],1],bins=binnumber,range=(0,1),normed=True,log=True,alpha=0.5,label='ChargeCorrect',facecolor='blue',edgecolor='black' )
plt.hist(y_pred[testset1.shape[0]:y_pred.shape[0],1],bins=binnumber,range=(0,1),normed=True,log=True,alpha=0.5,label='ChargeConfused',facecolor='green',edgecolor='black' )
plt.xticks(fontsize=20)
plt.yticks(fontsize=20)
