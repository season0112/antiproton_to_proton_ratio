...
'''
################ TrdLikelihood cut #####################
template_correct = template_correct[np.where(template_correct[:,1]>0.7)[0],:]
template_confused = template_confused[np.where(template_confused[:,1]>0.7)[0],:]
template_electron = template_electron[np.where(template_electron[:,1]>0.7)[0],:]
data_negative = data_negative[np.where(data_negative[:,1]>0.7)[0],:]
data_positive = data_positive[np.where(data_positive[:,1]>0.7)[0],:]
'''
'''
################ CC estimator cut #####################
...
TH_template_correct = TH2D("template_correct","Antiproton", CCbinningnumber,0,1,TRDbinningnumber,0.2,1.2);
TH_template_confused = TH2D("template_confused","Charge Confused Proton", CCbinningnumber,0,1,TRDbinningnumber,0.2,1.2);
TH_template_electron = TH2D("template_electron","Electron", CCbinningnumber,0,1,TRDbinningnumber,0.2,1.2); 
TH_data_negative = TH2D("data_negative","Negative Rigidity Events", CCbinningnumber,0,1,TRDbinningnumber,0.2,1.2);
TH_data_positive = TH2D("data_positive","Positive Rigidity Events", CCbinningnumber,0,1,TRDbinningnumber,0.2,1.2);
