...
binnings = np.array([16.6, 18.0, 19.5, 21.1, 22.8])
for binleft in range(binnings.shape[0]-1):
	...
	if Testsample is "proton":
        ...
        if correct_all.shape[0] == 0:
            correct_all = correct
            confused_all = confused
            testset1_all = testset1
            testset2_all = testset2
        else:
            correct_all = np.row_stack((correct, correct_all))
            confused_all = np.row_stack((confused, confused_all))
            testset1_all = np.row_stack((testset1, testset1_all))
            testset2_all = np.row_stack((testset2, testset2_all))
...
########################## Target Sample ############################
trainingset = np.r_[correct_all,confused_all]
y_proton = [1]*correct_all.shape[0]
y_CCproton = [0]*confused_all.shape[0]
...
######################### Test Target Sample  ##########################
testset = np.r_[testset1_all,testset2_all]
testsetMvA = testset[:,MvAresult] ## get the CCProton MvA results
testset1_pre = [1]*len(testset1_all)
testset2_pre = [0]*len(testset2_all)
...
protonpred = 0
CCprotonpred = 0
for protonpredcount in range(testset1_all.shape[0]):
	if np.argmax(y_pred[protonpredcount])==1:
		protonpred+=1
for CCprotonpredcount in range(testset1_all.shape[0],y_pred.shape[0]):	
	if np.argmax(y_pred[CCprotonpredcount])==0:
		CCprotonpred+=1
print("ChargeConrrect Prediction:"+str(protonpred)+" out of "+str(testset1_all.shape[0]))
print("ChargeConfused Prediction:"+str(CCprotonpred)+" out of "+str(testset2_all.shape[0]))	
...
plt.legend(loc='best',fontsize=20)
plt.subplot(223)
plt.hist(y_pred[0:testset1_all.shape[0],1],bins=binnumber,range=(0,1),log=True ) 
...
plt.subplot(224)
plt.hist(y_pred[testset1_all.shape[0]:y_pred.shape[0],1],bins=binnumber,range=(0,1),log=True ) 
...
protonMvApred = 0
CCprotonMvApred = 0
for protonMvApredcount in range(testset1_all.shape[0]):
	if testsetMvA[protonMvApredcount]>0.5:
		protonMvApred+=1		
for CCprotonMvApredcount in range(testset1_all.shape[0],y_pred.shape[0]):	
	if testsetMvA[CCprotonMvApredcount]<0.5:
		CCprotonMvApred+=1
plt.figure(figsize=(18,18))
plt.hist(testsetMvA[0:testset1_all.shape[0]],binnumber,range=(0,1),log=True, alpha=0.5,label='Proton',facecolor='blue',edgecolor='black'  )
plt.hist(testsetMvA[testset1_all.shape[0]:y_pred.shape[0]],binnumber,range=(0,1),log=True, alpha=0.5,label='CCProton',facecolor='green',edgecolor='black'  ) 
...
for protoniterate in range(binnumber):
	protonaccumulatecount=protonaccumulatecount+np.histogram(y_pred[0:testset1_all.shape[0],1],binnumber,range=(0,1))[0][binnumber-protoniterate-1]
	fractionoproton=protonaccumulatecount/(testset1_all.shape[0])
...
for CCprotoniterate in range(binnumber):
	CCprotonaccumulatecount=CCprotonaccumulatecount+np.histogram(y_pred[testset1_all.shape[0]:y_pred.shape[0],1],binnumber,range=(0,1))[0][binnumber-CCprotoniterate-1]
	rejectionCCproton=1.0/(CCprotonaccumulatecount/(testset2_all.shape[0]))
...
for protoniterateMvA in range(binnumber):
	protonaccumulatecountMvA=protonaccumulatecountMvA+np.histogram(testsetMvA[0:testset1_all.shape[0]],binnumber,range=(0,1))[0][binnumber-protoniterateMvA-1]
	fractionoprotonMvA=protonaccumulatecountMvA/(testset1_all.shape[0])
...
for CCprotoniterateMvA in range(binnumber):
	CCprotonaccumulatecountMvA=CCprotonaccumulatecountMvA+np.histogram(testsetMvA[testset1_all.shape[0]:y_pred.shape[0]],binnumber,range=(0,1))[0][binnumber-CCprotoniterateMvA-1]
	rejectionCCprotonMvA=1.0/(CCprotonaccumulatecountMvA/(testset2_all.shape[0]))
